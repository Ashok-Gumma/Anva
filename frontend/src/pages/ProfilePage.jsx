  ...
  return (
    <div className="max-w-4xl mx-auto p-3 sm:p-6 lg:p-8 space-y-6 font-minimal selection:bg-primary selection:text-primary-content">
      ...
      >
        ...
        <div className="px-6 sm:px-8 pt-4 pb-6 space-y-4 relative z-10">
          ...
          <div className="flex flex-wrap items-center gap-2 pt-1">
            ...
            <div className="px-3 py-1.5 bg-secondary/10 border border-secondary/20 rounded-xl text-xs font-bold text-secondary flex items-center gap-1.5">
              <BookmarkCheck className="w-3.5 h-3.5 shrink-0" /> {savedPosts.length} Saved
            </div>
          </div>
        </div>
      </motion.div>
      ...
      <div className="flex items-center justify-center p-1.5 bg-base-100 rounded-2xl border border-base-content/10 shadow-sm gap-1 font-minimal max-w-xl mx-auto">
        ...
        >
          <BookmarkCheck className="w-4 h-4" /> Saved ({savedPosts.length})
        </button>
      </div>
                      ...
                      <div className="pt-3 border-t border-base-content/10 space-y-2">
                        ...
                        <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                          {post.comments?.map((c, i) => (
                            <div key={i} className="p-3 bg-base-200/60 rounded-xl text-xs space-y-0.5">
                              <span className="font-bold text-base-content block">
                                {c.user?.fullName}
                              </span>
                              <p className="text-base-content/80 font-medium">{c.text}</p>
                            </div>
                          ))}
                        </div>
                      </div>
